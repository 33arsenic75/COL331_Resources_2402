...
void debugger3()
{
  ...
  cprintf("debugger: p = %p\n", p);
  cprintf("debugger: p->state = %d\n", p->state);
  cprintf("debugger: p->pid = %d\n", p->pid);
  cprintf("debugger: p->rss = %d\n", p->rss);
}
...
void print_rss()
{
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    if (DEBUG)
    {
      cprintf("PrintingRSS: p = %p\n", p);
      cprintf("PrintingRSS: p->state = %d\n", p->state);
      cprintf("PrintingRSS: p->pid = %d\n", p->pid);
      cprintf("PrintingRSS: p->rss = %d\n", p->rss);
    }
    if ((p->state == UNUSED))
      continue;
...
struct proc *find_victim_process(void)
{
  if (DEBUG)
  {
    cprintf("find_victim_process: called\n");
  }
  struct proc *p;
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    if (DEBUG)
    {
      cprintf("find_victim_process: p->pid = %d, p->state = %d, p->rss = %d\n", p->pid, p->state, p->rss);
    }
    if (p->pid < 2)
      continue;
...
uint find_victim_page(struct proc *p)
{
  ...
  for (i = 0; i < sz; i += PGSIZE)
  {
    if (DEBUG)
    {
      cprintf("find_victim_page: i = %d, sz = %d\n", i, sz);
    }
  ...
  if (DEBUG)
  {
    cprintf("find_victim_page: count = %d\n", count);
  }
  for (i = 0; i < sz; i += PGSIZE)
  {
    pte = walkpgdir(p->pgdir, (void *)i, 0);
